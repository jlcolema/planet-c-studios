/* global planetCStudiosBgColors, planetCStudiosPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( planetCStudiosBgColors ).forEach( function( context ) {
					planetCStudiosGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function planetCStudiosGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'planetcstudios-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#planetcstudios-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( planetCStudiosPreviewEls[ context ] ) {
				_.each( planetCStudiosPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		planetCStudiosGenerateColorA11yPreviewStyles( 'content' );
		planetCStudiosGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
