/* global planetCStudiosBgColors, planetCStudiosColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( planetCStudiosBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( planetCStudiosBgColors[ context ].color ) {
						backgroundColorValue = planetCStudiosBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( planetCStudiosBgColors[ context ].setting ).get();
					}
					planetCStudiosSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( planetCStudiosBgColors ).forEach( function( context ) {
			wp.customize( planetCStudiosBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					planetCStudiosSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		planetCStudiosSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				planetCStudiosSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function planetCStudiosSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = planetCStudiosColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Planet C Studios 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function planetCStudiosSetRetineLogoVisibility( visible ) {
