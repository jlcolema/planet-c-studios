...
class Media_Library_Organizer_API {
    ...
    private function request( $cmd, $method = 'get', $params = array() ) {
        ...
        switch ( $method ) {
             ...
             */
            case 'get':
                $result = wp_remote_get( $this->set_url( $cmd ), array(
             ...
             */
            case 'post':
                $result = wp_remote_post( $this->set_url( $cmd ), array(
                    'headers'   => $this->headers,
                    'body'      => ( $params != false ? json_encode( $params ) : '' ),
                    'timeout'   => $this->set_timeout(),
                ) );
                break;
        ...
        switch ( $http_response_code ) {
            ...
            case 403:
                return new WP_Error( 
                    ...
                    sprintf( 
                        __( 'HTTP Code %s', 'page-generator-pro' ),
                        $http_response_code
                    )
                ); 
                break;
